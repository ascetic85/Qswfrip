...
//	MakeDir(szPath);
    ...
    return fileInfo.dir().makeAbsolute();
...
UI32 CSWF::Load(const char *szFileName)
{
	...
	/*Read FILEHEADER*/
    if (fread(&fhFileHeader,1,sizeof(SWFFILEHEADER)-sizeof(UI32),pfileSWF)) {
        UI32 ui32;
        memset(&ui32, 0, sizeof(ui32));

        UI8 x0;
        fread(&x0, 1, sizeof(UI8),pfileSWF);
        ui32 = x0;

        UI8 x1;UI32 ui320;memset(&ui320, 0, sizeof(ui320));
        fread(&x1, 1, sizeof(x1),pfileSWF);
        ui320 = x1;
        ui32 |= (ui320) << 8;

        UI8 x2;memset(&ui320, 0, sizeof(ui320));
        fread(&x2, 1, sizeof(x2),pfileSWF);
        ui320 = x2;
        ui32 |= (ui320) << 16;

        UI8 x3;memset(&ui320, 0, sizeof(ui320));
        fread(&x3, 1, sizeof(x3),pfileSWF);
        ui320 = x3;
        ui32 |= (ui320) << 24;

        fhFileHeader.ulFileLength = ui32;

        printf("%u,%u,%u,%u,%u,%ul\n",fhFileHeader.bSignature1
               ,fhFileHeader.bSignature2
               ,fhFileHeader.bSignature3
               ,fhFileHeader.bVersion
               ,fhFileHeader.ulFileLength);
